...
pub async fn start_deepgram_stream(
    ...
    // We use multichannel=true and diarize=false because we are providing the channels ourselves.
    // Deepgram will provide channel_index in the results.
    let url = format!(
                        ...
                        if let Some(alternatives) = json["channel"]["alternatives"].as_array() {
                            for alt in alternatives {
                                ...
                                if is_final && !transcript.is_empty() && transcript != last_transcript {
                                    ...
                                    let _ = transcript_tx.send(payload.clone());
